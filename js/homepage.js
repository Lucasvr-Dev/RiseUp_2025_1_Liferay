...
async function carregarEventos() {
  ...
  try {
    ...
    const eventos = await response.json(); // Limpa os cards fixos existentes

    track.innerHTML = ""; // Se não houver eventos cadastrados
    ...
    eventos.forEach((evento) => {
      ...
      card.classList.add("card"); // Imagem padrão

      const img = document.createElement("img");
      img.src = "/assets/pictures/liferay-devcon.jpg"; // Você pode mudar para evento.imagem se tiver no banco
      img.alt = evento.nome || "Evento Liferay"; // ========================================== // ========= INÍCIO DA MODIFICAÇÃO ======== // ========================================== // 1. Título do evento (H3)

      const h3 = document.createElement("h3"); // 2. O Link (A) que vai DENTRO do H3
      const linkNome = document.createElement("a"); // Linka para a página de detalhes com o ID do evento // (Certifique-se que seu 'evento' tem a propriedade 'id')
      linkNome.href = `/detalhes-evento.html?id=${evento.id}`;
      linkNome.textContent = evento.nome || "Evento sem nome"; // Estilo para o link não ficar azul e sublinhado
      linkNome.style.color = "inherit";
      linkNome.style.textDecoration = "none"; // Coloca o link <a> dentro do <h3>
      h3.appendChild(linkNome); // 3. Data do evento (P)

      const data = document.createElement("p"); // Você pode querer formatar essa data
      data.textContent = evento.data || ""; // 4. Descrição do evento (P)

      const descricao = document.createElement("p");
      descricao.textContent = evento.descricao || ""; // Monta o card

      card.appendChild(img);
      card.appendChild(h3); // Adiciona <h3><a>Nome</a></h3>
      card.appendChild(data); // Adiciona <p>Data</p>
      card.appendChild(descricao); // Adiciona <p>Descrição</p> // ========================================== // =========== FIM DA MODIFICAÇÃO =========== // ========================================== // Adiciona o card ao carrossel
    ...
    console.error("Erro ao carregar eventos:", error); // Adiciona uma mensagem de erro visual para o usuário
